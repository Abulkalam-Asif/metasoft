...
const LargeScreenNav = () => {
  ...
  return (
    <div>
      <nav className={`fixed ${isDarkMode?"text-white":"text-black"} top-0 z-50 w-full font-roboto whitespace-nowrap animate-fade-in`}>
        <div className="max-w-[1550px] backdrop-blur-md hidden lg:flex gap-8 w-4/5 mx-auto border items-center justify-between text-[var(--text-primary)] border-[var(--text-primary)]/50 rounded-3xl px-6 py-4 my-5">
          <div className="w-max flex gap-x-6 items-center">
            <Link className={`${isDarkMode ? 'brightness-100' : 'brightness-10'}`} to={"/"}>
              <img src={logo} alt="logo" className="h-8 w-auto" />
            </Link>

            <div className="relative">
              <FaSearch className={`absolute top-1/2 -translate-y-1/2 left-4  ${isDarkMode?"text-[#666b75]":"text-[#ffffff]"}  text-lg`} />
              <input
                type="text"
                name=""
                id=""
                placeholder="Search"
                className={`w-full ${isDarkMode?"bg-[#21222d] placeholder:text-[#666b75]":"bg-[#848485] placeholder:text-[#ffffff]"} text-white py-1 pl-12 pr-4 rounded-xl `}
              />
            </div>
      ...
      <button
        onClick={toggleTheme}
        className="fixed left-10 top-13 -translate-y-1/2 p-3 rounded-full bg-[var(--bg-secondary)] hover:bg-[var(--bg-primary)] border border-[var(--text-primary)]/20 transition-colors z-50"
      >
        {isDarkMode ? (
